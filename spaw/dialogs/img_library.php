...
<?php
$imglib = $_POST['lib'] ?? '';
if (empty($imglib) && isset($_GET['lib'])) {
    $imglib = $_GET['lib'];
}
...
<head>
    ...
    <?php if ('Gecko' == SPAW_Util::getBrowser()) { ?>
            ...
            <tr>
                <td valign="top" align="left">
                    ...
                    <select name="imglist" id="imglist" size="15" class="input" style="width: 150px;"
                        ...
                        <?php
                        /*
                          echo "<br>libtype:".$libtype;
                          if (!empty($libtype) && $libtype == 'XoopsImage'){
                            global $xoopsDB;
                        ...
                        if ($d) {
                            while (false !== ($entry = $d->read())) {
                                $ext = mb_strtolower(mb_substr(mb_strrchr($entry, '.'), 1));

                                if (is_file($_root . $imglib . $entry) && in_array($ext, $spaw_valid_imgs, true)) {
                                    ?>
